...
for (let i = 0; i < paths.length; i++) {
  if (paths[i + 1]) {
    interpolators.push(
      interpolate(paths[i], paths[i + 1], { maxSegmentLength: 0.1 })
    )
  }
}
...
export default class Opening extends React.Component {
  ...
  goNext = () =>
    this.setState(({ index, interpolators }) => ({
      index: index + 1 >= interpolators.length ? 10 : index + 1,
    }))
  render() {
    const { interpolators, index } = this.state
    const interpolator = interpolators[index] === undefined ? "" : interpolators[index]
    if (interpolator === "") {
      return <div>ddd</div>;
    }
    return (
